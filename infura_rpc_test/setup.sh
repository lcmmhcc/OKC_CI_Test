...
#send tx
curl -X POST http://127.0.0.1:8545 -s -H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","method":"personal_unlockAccount","params":["0xbbE4733d85bc2b90682147779DA49caB38C0aA1F", "", 30],"id":1}'
export txHash=$(curl -X POST http://127.0.0.1:8545 -s -H 'Content-Type: application/json' -d '{"jsonrpc":"2.0","method":"eth_sendTransaction","params":[{"from":"0xbbE4733d85bc2b90682147779DA49caB38C0aA1F", "to":"0x2727D250a14aa69FdcD702C82b314717aBa4A6d0", "value":"0x16345785d8a0000", "gas":"0x52080", "gasPrice":"0x55ae82600"}],"id":1}' | jq '.result')

sleep 4
txRes=$(curl -X POST http://127.0.0.1:8545 -s -H 'Content-Type: application/json' -d "{\"jsonrpc\":\"2.0\",\"method\":\"eth_getTransactionByHash\",\"params\":[$txHash],\"id\":1}" | jq '.result')
